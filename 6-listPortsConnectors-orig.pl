...
print "DEBUG: Parent From Block Part: $parentFromBlockPart\nParent From Block: $parentFromBlock\n\n\n";
	...
	foreach(@portNames_arr) {
		my ($fromPortName, $fromPortID) = split(/\|\|/,$_);
		print "\t\t\t\"$fromPortName\":\n";
		print "\t\t\t\t{\n";
		my $portDetails = printPortDetails($fullPath, $fromPortName, $fromPortID, $parentFromBlockPart, $parentFromBlock);
		print "\t\t\t\t}\n";
	}
	print "\t\t}\n";
	print "\t\t]\n";
...
if ($otherPart ne ".") {
	...
	print "\n\nPorts for part $otherPart ($parentToBlock):\n";
	my @portNames_arr = split(/::/,$toPorts);
	foreach(@portNames_arr) {
		chomp($_);
		my ($toPortName,$portID)= split(/\|\|/,$_);
		print "$toPortName\n";
		print "Connectors:\n";
		my $connector = queryConnectors($_, $fromPorts, $parentFromBlock, $parentFromBlockPart); 
	}
...
sub queryConnectors{
	...
	my($toPortName,$toPortID)=split(/\|\|/,$toPort); 
	...
	my $blockFiles = qx/ find $fullPath \-type f  \| xargs -n1 awk \'\/<value>$partID<\\\/value>\/\,\/<_name type=\"a\">$parentBlock<\\\/_name>\/ \{printf FILENAME \"  \"\; print\}\'  / ;
	...
	my $connectorName = getConnectorName($toPortID, $fromPortID, $fileContents);
	
	
	print "\tfrom $fromPortName to $toPortName:\t==>"; 
	print "\t$connectorName\n";
	}
...
sub printPortConnectors{
	...
	foreach(@connectorInfo_arr){
		...
		print "\t\t\t\t\t\"$connectorDir1\":\"$connectorPartName1\"\n";
		print "\t\t\t\t\t\"$connectorDir2\":\"$connectorPartName2\"\n";
		print "\t\t\t\t\t\"name\":\"$connectorName\"\n";
		...
		if ($inConnector eq "true"){
			...
			if ($inToPort eq "true") {
				if (index($_,$portID)!=-1) {
					...
					else {$connectionString = $connectionString . "???" . $prospectConnectorName . "CONNECTOR_SEPERATOR" . $toPart . "CONNECTOR_SEPERATOR" . $fromPart;}
				}
			}
			
			if ($inFromPort eq "true") {
				if (index($_,$portID)!=-1) {
					$toPart = $prospectFromPart . ":::To";
					$fromPart = $prospectToPart . ":::From";
					if ($connectionString eq ""){$connectionString = $prospectConnectorName . "CONNECTOR_SEPERATOR" . $toPart . "CONNECTOR_SEPERATOR" . $fromPart;}
